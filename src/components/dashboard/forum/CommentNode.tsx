...
import { formatDate } from './markdown';
...
const countAllReplies = (node: ForumComment): number => {
  let count = 0;
  if (node.replies) {
...
export function CommentNode({ comment, depth = 0, onAddReply, onDeleteComment }: CommentNodeProps) {
  ...
  return (
    <div className="space-y-3 relative" style={{ marginLeft: depth > 0 ? '1.5rem' : '0px' }}>
      ...
      {collapsed ? (
        ...
        <>
          <Card variant="accent" animated={false} className="p-5 space-y-3 relative hover:border-[#35b9f1]/10 transition-all duration-300 rounded-xl">
